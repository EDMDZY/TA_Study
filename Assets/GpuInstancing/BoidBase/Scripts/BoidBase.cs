...
{
    static public List<BoidBase> boids;     //实例化Boid 的表

    public Vector3 velocity;        //当前速度
    public Vector3 newVelocity;     //下一帧中的速度
    public Vector3 newPosition;     //下一帧中的位置

    public List<BoidBase> neighbors;        //附近所有的 Boid 的表
    public List<BoidBase> collisionRisks;   //距离过近的所有 Boid 的表(具有碰撞风险，需要处理)
    public BoidBase closest;                //最近的 Boid
    ...
    private void Awake()
    {
        ...
        Vector3 randPos = Random.insideUnitSphere * BoidBaseSpawner.S.spawnRadius;
        ...
        velocity *= BoidBaseSpawner.S.spawnVelcoty;

        //初始化两个List
        neighbors = new List<BoidBase>();
        collisionRisks = new List<BoidBase>();
    ...
    private void Update()
    {
        ...
        List<BoidBase> neighbors = GetNeighbors(this);
        ...
        newVelocity += neighborVel * BoidBaseSpawner.S.velocityMatchingAmt;
        ...
        */
        ...
        newVelocity += neighborCenterOffset * BoidBaseSpawner.S.flockCenteringAmt;
        ...
        */
        ...
        if (collisionRisks.Count > 0)   //处理 最近的boid 表
        {
            ...
            newVelocity += dist * BoidBaseSpawner.S.collisionAvoidanceAmt;
        }

        //跟随鼠标光标：无论距离多远都向鼠标光标移动
        dist = BoidBaseSpawner.S.mousePos - this.transform.position;

        //若距离鼠标光标太远，则靠近；反之离开(修改新速度)
        if (dist.magnitude > BoidBaseSpawner.S.mouseAvoiddanceDsit)
            newVelocity += dist * BoidBaseSpawner.S.mouseAtrractionAmt;
        else
            newVelocity -= dist.normalized * BoidBaseSpawner.S.mouseAvoidanceAmt;
    ...
    private void LateUpdate()
    {
        //使用线性插值法
        //基于计算出的新速度 进而修改 当前速度
        velocity = (1 - BoidBaseSpawner.S.velocityLerpAmt) * velocity + BoidBaseSpawner.S.velocityLerpAmt * newVelocity;

        //确保 速度值 在上下限范围内(超过范围就设定为范围值)
        if (velocity.magnitude > BoidBaseSpawner.S.maxVelocity)
            velocity = velocity.normalized * BoidBaseSpawner.S.maxVelocity;
        if (velocity.magnitude < BoidBaseSpawner.S.minVelocity)
            velocity = velocity.normalized * BoidBaseSpawner.S.minVelocity;
    ...
    public List<BoidBase> GetNeighbors(BoidBase boi)
    {
        ...
        foreach (BoidBase b in boids)
        {
            ...
            if (dist < BoidBaseSpawner.S.nearDist)  //处在附近的 boid 范围
                neighbors.Add(b);

            if (dist < BoidBaseSpawner.S.collisionDist) //处在最近的 boid 范围(有碰撞风险)
    ...
    public Vector3 GetAveragePosition(List<BoidBase> someBoids)
    {
        Vector3 sum = Vector3.zero;
        foreach (BoidBase b in someBoids)
    ...
    public Vector3 GetAverageVelocity(List<BoidBase> someBoids)
    {
        Vector3 sum = Vector3.zero;
        foreach (BoidBase b in someBoids)
            sum += b.velocity;
