	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag (v2f i) : SV_Target
			{
				...
				half3 env_IBLspec = DecodeHDR(color_cubemap, _CubeMap_HDR);//确保在移动端能拿到HDR信息

				// 1. 球谐环境光照
   				half3 env_SH = ShadeSH9(float4(normal_dir,1.0));
   				
   				// 2. 添加主光源漫反射
   				half3 light_dir = normalize(_WorldSpaceLightPos0.xyz);
   				half NdotL = saturate(dot(normal_dir, light_dir));
   				half3 main_light = _LightColor0 * NdotL;
   				
   				// 3. 组合光照
   				half3 final_color = (env_SH + env_IBLspec) * ao * _Tint.rgb * _Expose ;
